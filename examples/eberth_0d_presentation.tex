...
\midrule
$\bar Q$ = 0.016 ml/s & carotid bed $R$ \\
MAP = 92 mmHg & systemic $R$ \\
PP = 42 mmHg & aortic $C$ \\
PI = 1.16 & carotid bed $C$ \\
\bottomrule
\end{tabular}
\end{column}
...
target (measured) & knob that sets it \\
\midrule
$\bar Q$ = 0.022 / 0.012 & RCCA / LCCA bed $R$ \\
MAP = 86 / 77 & band $S$ (A$\to$B drop) $+$ sys $R$ \\
PP = 56 / 27 & aortic $C_a, C_b$ \\
PI = 3.11 / 1.65 & RCCA / LCCA bed $C$ \\
\bottomrule
\end{tabular}
\end{column}
\end{columns}
...
Bed resistances are \emph{pinned} by $(\bar Q,\text{MAP})$; the actively-tuned
knobs are the bed compliances ($\to$ PI) and, for banded, the band $S$ ($\to$ the
A$\to$B pressure drop). \; Fixed for both groups: HR (7.17 / 6.09), cardiac
output CO, carotid geometry, blood $\mu,\rho$, wall moduli $E$, ejection shape.
\end{frame}
...
\\[0.2em]
\footnotesize DOF-balanced (12 targets / 12 knobs). Cost: bed compliance differs
...
\\[0.3em]
\footnotesize Carotid bed $R$ sets the flow split (RCCA-B low $\to$ more flow);
the banded carotid-bed \emph{compliance} is $\sim$3--4$\times$ the control's --- a
\textbf{fitted} difference, not measured (opposite to what autoregulation would
do). Systemic $R$ is essentially unchanged; aortic $C$ (both the proximal and
distal segment, either side of the band) is \textbf{not tuned in either group} ---
same 0.64/0.36 split of the measured central aortic compliance for control and
banded alike.
\end{frame}
