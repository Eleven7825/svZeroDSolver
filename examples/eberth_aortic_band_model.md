...
Model card for the two configs in this folder — the **acute**
[`eberth_aortic_band_acute.json`](./eberth_aortic_band_acute.json) and the
...
physiological parameters and the **same** calibrated band; they differ **only**
in carotid-wall geometry (acute = baseline CCA both sides; chronic = Table-1
remodeled). See §7 for the acute/chronic comparison and §8 for the full parameter
...
Both files are built by `make_configs.py` from the **same physiological
parameters** and the **same calibrated band** (`stenosis_coefficient = 53.8`);
they differ **only** in carotid-wall geometry:

| File | Carotid walls | Represents |
|------|---------------|------------|
| `eberth_aortic_band_acute.json` | **baseline CCA** both sides (496 µm / 24.8 µm) | **acute** redistribution — the immediate *stimulus* |
| `eberth_aortic_band_chronic.json` | **Table-1 remodeled** (RCCA-B 633/88, LCCA-B 482/42) | **chronic** 5–8-week state — stimulus *plus* wall adaptation |

The band spacer is the same physical object in both states, so the stenosis was
...
| | RCCA-B PI | LCCA-B PI | RCCA-B PP | LCCA-B PP |
|---|---:|---:|---:|---:|
| **Acute** (baseline carotids) | 2.85 | 1.60 | 60 | 36 |
| **Chronic** (remodeled carotids) | 2.88 | 1.53 | 60 | 36 |
| Paper Table 1 (chronic) | 3.11 | 1.65 | 56 | 27 |

The two states are **nearly identical**: the ~3.5× RCCA-B wall thickening barely
changes local pulsatility, because the carotid wall compliance (~1e-6 ml/mmHg) is
tiny next to the aortic-buffer and RCR-bed compliances. So in this 0D model the
pulsatility asymmetry is driven **almost entirely by band position**, and is
present **immediately** on banding — the wall remodeling is the *response* to
that pulsatile stimulus, not its cause, exactly as Eberth argue.
